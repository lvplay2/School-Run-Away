...
public class SimpleFreeLookCam : PivotBasedCameraRig
{
	...
	private void OnEnable()
	{
		...
		m_LookAngle = base.transform.localRotation.eulerAngles.y;
		float x = m_Pivot.localRotation.eulerAngles.x;
		m_TiltAngle = ((!(x > 180f)) ? x : (x - 360f));
	...
	private void HandleRotationMovement()
	{
		if (!(Time.timeScale < float.Epsilon))
		{
			...
			{
				m_TiltAngle = ((!(axis2 > 0f)) ? Mathf.Lerp(0f, m_TiltMax, 0f - axis2) : Mathf.Lerp(0f, 0f - m_TiltMin, axis2));
			}
			else
			{
